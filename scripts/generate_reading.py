                ...
                if not isinstance(pairs, list) or not pairs: errors.append(f"{b}.pairs is required")
                    ...
                    for k, pair in enumerate(pairs, 1):
                        if not isinstance(pair, list) or len(pair) < 4:
                            errors.append(f"{b}.pairs[{k}] must be [q,before,answer,after] or [q,before,answer,between,q2,answer2,...,after]")
                            continue
                        if len(pair) > 4:
                            if len(pair) < 6:
                                errors.append(f"{b}.pairs[{k}] multi-blank pair is too short; question numbers: {[x for x in pair if isinstance(x, int)]}")
                            for pos in range(4, len(pair) - 1, 2):
                                if not isinstance(pair[pos], int):
                                    errors.append(f"{b}.pairs[{k}] expected question number at index {pos}; got {pair[pos]!r}")
...
def iter_summary_blanks(pair):
    """Yield (q, before, answer, after) for normal or compact multi-blank summary pairs.

    Supported forms:
    - [q, before, answer, after]
    - [q, before, answer, between, q2, answer2, after]
      and repeated [between, qN, answerN] chunks, with optional final after.
    """
    if len(pair) <= 4:
        yield pair[0], pair[1], pair[2], pair[3]
        return
    q_num, before, answer = pair[0], pair[1], pair[2]
    idx = 3
    while idx < len(pair):
        between = pair[idx]
        if idx + 2 < len(pair) and isinstance(pair[idx + 1], int):
            yield q_num, before, answer, between
            q_num, before, answer = pair[idx + 1], "", pair[idx + 2]
            idx += 3
        else:
            yield q_num, before, answer, between
            break
...
def build_summary(num_pairs, reveals=None):
    ...
    for pair in num_pairs:
        for q_num, before, answer, after in iter_summary_blanks(pair):
            ...
            r = reveals[reveal_i] if reveals and reveal_i < len(reveals) else answer
...
def build_heading_slot(p_num, label, q_num, answer, answer_text=''):
    ...
    return (f'<div class="heading-drop-row" data-para="{a(label)}" data-q="{a(q_num)}">'
            f'<div class="heading-drop-label">Q{h(q_num)} / Paragraph {h(label)}</div>'
            ...
            f'<span class="slot-placeholder">拖入 heading / Drop heading</span></div></div>')
